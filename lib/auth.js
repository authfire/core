...
const useCurrentUser = () => {
    const { isLoading, auth } = (0, const_1.useFirebase)();
    const [isUserLoading, setIsUserLoading] = (0, react_1.useState)(true);
    const [user, setUser] = (0, react_1.useState)(auth?.currentUser || null);
    const [idTokenVerified, setIdTokenVerified] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        if (isLoading || !auth)
            return;
        (0, auth_1.onAuthStateChanged)(auth, (newUser) => {
    ...
    (0, react_1.useEffect)(() => {
        if (isUserLoading || !user || !const_1.idTokenVerificationUrl) {
